...
#
# Description:
# 	SQLite support library for the DFS project. Database info for the 
#       metadata server.
#
...
class mds_db:
	...
	def GetNode(self, nid):
		"""Get the entry for the node with nid."""
		query = """SELECT * FROM dnode WHERE nid=%s""" % (nid)
		try:
			self.c.execute(query)
		except:
			return None
	...
	def AddBlockToInode(self, fname, blocks):
		...
		print "fid: %s, dummy1: %s" % (fid, dummy1)
		if not fid:
			return None
